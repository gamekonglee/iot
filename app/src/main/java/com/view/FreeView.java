...
//        Log.w("layoutbase",l+","+t);
        if(!isFirst){
            ...
            bottom = b;
            maxWidth = right+100;
            maxHeight = bottom+100;// 此时减去状态栏高度 注意如果有状态栏 要减去状态栏 如下行 得到的是可活动的高度
            minWidth = left-100;
            minHeight = top-100;
        }
//        position[0]=l;
//        position[1]=t;
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE: // 滑动动作处理 记录离开屏幕时的 moveX  moveY 用于计算距离 和 判断滑动事件和点击事件 并作出响应
                    ...
                    if (Math.abs(moveX) > 3 ||Math.abs(moveY) > 3) { // 偏移量的绝对值大于 3 为 滑动时间 并根据偏移量计算四点移动后的位置
                        ...
                        if(l<minWidth){ // left 小于 0 就是滑出边界 赋值为 0 ; right 右边的坐标就是自身宽度 如果可以划出边界 left right top bottom 最小值的绝对值 不能大于自身的宽高
                            l=minWidth;
                            r=l+width;
                            isLeft=true;
                        ...
                        if(onSlidingDirectionListener!=null){
                            if(isTop){
                                if(isLeft){
                                    onSlidingDirectionListener.up_left();
                                }else if(isRight){
                                    onSlidingDirectionListener.up_right();
                                }else {
                                    onSlidingDirectionListener.up();
                                }
                            }else if(isBottom){
                                if(isRight){
                                    onSlidingDirectionListener.down_right();
                                }else if(isLeft){
                                    onSlidingDirectionListener.down_left();
                                }else {
    ...
    OnSlidingDirectionListener onSlidingDirectionListener;
